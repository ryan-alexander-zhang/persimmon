...
public class LayerChecksTest {
  ...
  static final ArchRule layered_architecture_must_follow_dependency_direction =
    layeredArchitecture()
      ...
      .layer("Start").definedBy(START_PACKAGE)
      .whereLayer("Domain").mayOnlyBeAccessedByLayers("App", "Infra", "Start")
      .whereLayer("App").mayOnlyBeAccessedByLayers("Adapter", "Start")
      .whereLayer("Adapter").mayOnlyBeAccessedByLayers("Start")
      .whereLayer("Infra").mayOnlyBeAccessedByLayers("Start")
      .whereLayer("Start").mayNotBeAccessedByAnyLayer()
      .because("Layer boundaries must remain strict to keep domain purity and enforce dependency inversion.");

  @ArchTest
  static final ArchRule domain_must_not_depend_on_app_adapter_infra_or_start =
    noClasses()
      .that().resideInAPackage(DOMAIN_PACKAGE)
      .should().dependOnClassesThat().resideInAnyPackage(APP_PACKAGE, ADAPTER_PACKAGE, INFRA_PACKAGE, START_PACKAGE)
  ...
  static final ArchRule app_must_not_depend_on_adapter_or_infra_or_start =
    noClasses()
      ...
      .should().dependOnClassesThat().resideInAnyPackage(ADAPTER_PACKAGE, INFRA_PACKAGE, START_PACKAGE)
      .because("Application layer orchestrates use cases but must not depend on inbound adapters or infrastructure.");

  @ArchTest
  static final ArchRule adapter_must_not_depend_on_domain_or_infra_or_start =
    noClasses()
      .that().resideInAPackage(ADAPTER_PACKAGE)
      .should().dependOnClassesThat().resideInAnyPackage(DOMAIN_PACKAGE, INFRA_PACKAGE, START_PACKAGE)
      .because("Inbound adapters must call the application layer only, and must not reach into domain or infra.");

  @ArchTest
  static final ArchRule infra_must_not_depend_on_adapter_or_app_or_start =
    noClasses()
      .that().resideInAPackage(INFRA_PACKAGE)
      .should().dependOnClassesThat().resideInAnyPackage(ADAPTER_PACKAGE, APP_PACKAGE, START_PACKAGE)
      .because("Infrastructure implements ports and technical details; it must not depend on app/adapter/start.");
}
