...
| The npm package in this repository needs Node.js ≥ 23.6 — type stripping for its own `npm start` and tests, which import `src/*.ts`; the shipped package carries compiled JS in `lib/` because Node does not strip under `node_modules` (issue-00029); `node-pty` native module | `package.json` |
| One artifact and one implementation language: the command is this package's bin, no second language and no second product | [decision-00020](docs/decision/decision-00020-unified-go-cli.md) §2 |
...
flowchart LR
  ...
  H[Document owner<br/>shell] --> C[persimmon command<br/>npm bin]
...
├── bin/persimmon.js         # thin entry: parses argv and calls lib/cli.js — the coverage gate sees src/, not bin/
...
│                            #   watcher, server (HTTP/WS API); cli (subcommand dispatch, handshake,
│                            #   add/remove/list); scaffold (new/update/list-langs);
│                            #   workspaceRegistry, workspaceAvailability
...
There is one artifact: the npm package `@ryan-alexander-zhang/persimmon` with the bin `persimmon`. The bin dispatches every subcommand and the service, the scaffold is a module under `src/`, and the workspace registry has a single implementation there ([decision-00020](docs/decision/decision-00020-unified-go-cli.md) §2).
...
| Command startup: probe, then join a running process or serve | design-00003 §8 |
...
Supported platforms are linux and darwin. Windows is out of scope ([decision-00020](docs/decision/decision-00020-unified-go-cli.md) §2 row 9): the template carries a `CLAUDE.md -> AGENTS.md` symlink that Windows cannot create without Developer Mode or elevation, unarchiving shells out to `tar` which Windows does not guarantee, and node-pty has never been exercised there.

Local only. A user runs the command from the npm package — `npx @ryan-alexander-zhang/persimmon` or `npm i -g @ryan-alexander-zhang/persimmon`. No version is published yet, so today both forms mean a local `npm pack` tarball or `npm link`; developing this repository is `npm run build` once, then `npm start` from anywhere inside a repository that has `whiteboard.config.yaml`.

There is one artifact and it is not published yet: the owner has decided to hold off on a first version, so `version` stays `0.0.0-dev` and the repository carries no release workflow ([decision-00020](docs/decision/decision-00020-unified-go-cli.md) §2). When a version is cut it is a single `npm publish` — with one artifact there is nothing to pair, no checksums to verify and no two-step publish order.

`.github/workflows/ci.yml` runs Node build, typecheck and tests on every push and pull request. The remaining open gates are listed in [CODE_QUALITY.md](CODE_QUALITY.md) §2.
...
| [decision-00020](docs/decision/decision-00020-unified-go-cli.md) | One `persimmon` command: this package's bin owns every command-line entry, as a single npm artifact |
...
| Test coverage | Any code change, the command included | Lines, branches, functions ≥ 90% over `src/` and `web/src/` (TESTING.md) |
...
The risk that the registry file contract drifts between two implementations is gone, not mitigated: [decision-00020](docs/decision/decision-00020-unified-go-cli.md) §2 leaves one implementation of the registry and one of the availability judgement, both in `src/`, so there is no second side to drift from.
