...
**§5 对 `record-00035` 的指令，计数据实修正**：§5 写「16 行『待人工实测』中的
15 行随其需求作废」。正文的历史表述保留，但那个计数不准：这 16 行里属
`spec-00012` 的是 15 行，按本文 §5 自己的逐条裁定（`FR-3` / `FR-4` / `FR-10`
是**改写**，`FR-5` / `FR-11` 是**作废**），其中**只有 8 行随其需求或所采样的
机制作废**（`AC-5.1` / `AC-5.2`、`AC-11.1`…`AC-11.5`，加 `AC-3.2`——`FR-3`
虽是改写，但该条采样的是转发这一层的正确性，单进程下两个流合一、断言退化为
同义反复，故退出验收集，stdout 那一半由改写后的 `AC-3.1` 采样），另 **7 行是
改写后继续存在**（`AC-3.1` / `AC-3.3`、`AC-4.1` / `AC-4.2` / `AC-4.3`、
`AC-10.1` / `AC-10.2`），须由后续 plan 的新验收记录重新取证；第 16 行
`spec-00011-AC-20.1` 是 Given 改写（`install.sh` 装入 PATH → npm bin），也不是
作废。追注后的准确分类以 `record-00035` 第三十二轮追注第 2 条为准。
