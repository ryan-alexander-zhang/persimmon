...
（FR-15）承担。墓碑仍写成完整的条目
声明形态是**故意的**：唯一能让一行不被解析为验收条目的办法是不以粗体 id 起头，
而那会让引它们的 `record` 行变成断链（`decision-00005` §1 记的正是这一类事故）。
**第三十三轮起这不再只靠本段散文**：十六条的归属标注各加了第二个 token `作废`
（`docs/spec/README.md` 的条目文法，`spec-00001-FR-32`），机器据此把它们排除在
「计入的 AC」之外，本段与标记同义。条目声明行今天还没有这个标记位，故
`FR-5` … `FR-8`、`FR-11` 这五条整条作废的需求在检视面板上的读数仍不对，
见 `issue-00040`。
...
- **spec-00012-AC-5.2** (spec-00012-FR-5, 作废)
...
- **spec-00012-AC-11.2** (spec-00012-FR-11, 作废)
  Given 原断言「校验和文件取到了但其中没有该归档那一行时，同样中止安装」
  When 同 `AC-11.1`
  Then 该断言随之失效，自第三十二轮起退出验收集，**无替代条目**——id 就地留存，
  供 `record-00035` 的既有引用解析
- **spec-00012-AC-11.3** (spec-00012-FR-11, 作废)
  Given 原断言「该归档那一行不合式（校验和字段不是一串十六进制）时，同样中止
  安装」
  When 同 `AC-11.1`
  Then 该断言随之失效，自第三十二轮起退出验收集，**无替代条目**——id 就地留存，
  供 `record-00035` 的既有引用解析
- **spec-00012-AC-11.4** (spec-00012-FR-11, 作废)
  Given 原断言「校验和文件取不到时，脚本给出一句未能校验的警告并照常装好」
  When 同 `AC-11.1`
  Then 该断言随之失效，自第三十二轮起退出验收集，**无替代条目**——id 就地留存，
  供 `record-00035` 与 `issue-00038` 的既有引用解析
- **spec-00012-AC-11.5** (spec-00012-FR-11, 作废)
