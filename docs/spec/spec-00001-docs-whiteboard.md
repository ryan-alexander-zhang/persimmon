...
- canonical terms 见 `CONTEXT.md`：白板、节点、评审动作、接收、澄清、审计、
  ...
  不可用 workspace、切换器（第二十八轮）、配置校验（第二十九轮）。
...
- 第二十九轮的修订源：[spec-00011-multi-workspace](spec-00011-multi-workspace.md)
  第二十九轮——FR-15 的降级清单补 `spec-00003-AC-3.5`（第二十八轮漏列，而
  `spec-00011` §1 已指定本清单为该降级的单一来源），及 Q29.2 的正名
  「启动校验」→「配置校验」一处措辞。
- 第三十三轮的修订源：[issue-00039](../issue/issue-00039-a-tombstone-ac-holds-its-item-uncovered.md)
  与其实证 [record-00036](../record/record-00036-persimmon-single-npm-artifact-acceptance.md)
  第 10 项——第三十一/三十二轮起，需求作废的 AC 按「id 就地留存」的墓碑约定不删
  （`spec-00011-AC-20.3` / `21.3` / `21.4`、`spec-00012` 十六条），而条目文法只有
  一种 AC 形态、FR-32 把「被声明」直接等同于「须验证」，墓碑因此照常被当成一条
  欠验收行的 AC。`plan-00034` 的 `resolved` 门（FR-52，证据集另限为 `parent`
  指向该 plan 的 record）据此报出四条它不欠的缺口：`spec-00012-FR-3` / `FR-10`、
  `spec-00011-FR-20` / `FR-21`，读数为 `uncovered`。逐条动作：
  - 条目文法（`docs/spec/README.md`）：AC 的归属标注增开第二个逗号分隔 token
    `作废`，标出**墓碑 AC**；未知的第二 token 不成立，该 AC 归 FR-33 的「无法
    归属」。
  - FR-32：在三态之前先定义**计入的 AC**（该条目下未标 `作废` 的 AC），三态
    原文措辞不变、只把「AC」换成「计入的 AC」，条目类型仍是 Ubiquitous——墓碑
    不是新加的条件，是同一个不变式换了作用集。另明写墓碑 AC 上的引用行如何计：
    非 `pass` 仍入「未通过」，`pass` 惰性。新增 `AC-32.11` … `AC-32.14`。
  - FR-33：无法归属的判定不变，只补一句它的新入口（未知的第二 token）与一条
    `AC-33.4`。
  - FR-31 与 FR-52 **不动**：AC 计数照旧含墓碑（墓碑归属得上，就计数），门复用
    FR-32 的三态、随之生效。
  - 本轮**不做 FR 侧的 `作废` 标记**（`spec-00012-FR-5` … `FR-8`、`FR-11` 那类
    整条作废的需求），也不引入「不携带覆盖状态」的第四态：那要连改
    `spec-00002-FR-10` / `FR-11` 的全局覆盖计数、`design-00001` §7 的 API、
    `design-00002` §2/§9 的呈现与 `web/src/coverageMarks.ts`，本轮不背。整条
    作废的那五条 FR 其 AC 全为墓碑，按本轮的 FR-32 读「未覆盖」；它们不在任何
    plan 的 `implements` 里，不挡门。面板上这一读数留给
    [issue-00040](../issue/issue-00040-a-dead-requirement-still-reads-as-a-gap.md)，
    本轮不裁定。
  - **不新开 FR-40 的诊断类**：`issue-00039` §8 问「条目文法每加一位，解析诊断
    是否也须跟着加一条」。裁定不加——未知的第二 token 已有归宿（FR-33 的无法
    归属，与写错的归属 id 同类，读者看到的是同一个区），再开一类只是把同一件
    事说两遍。
  - 据实校正一处前提：**面板今天把这些条目读作「未通过」而不是「未覆盖」**——
    `record-00035` 有 16 行结果为 `待人工实测` 的验收行落在它们上，非 `pass`
    优先于未覆盖（FR-32）。`uncovered` 只在门另限的证据集（`parent` 指向
    `plan-00034` 的 record）下出现。同一根因的两个可见面，后者归 issue-00040。
...
- **spec-00001-FR-15** (Unwanted) 系统应在打开每个 workspace 时读取并校验其
  ...
  **第二十八轮（`spec-00011-FR-6`/`FR-9`/`FR-12`）：拒绝的作用域由进程改为
  ...
  `spec-00003-FR-3`/`AC-3.4`/`AC-3.5`（`max_sessions`；第二十九轮补 `AC-3.5`——
  它是该组的满足半边，正文写着「第二十八轮改写：原「正常启动」」，本清单当轮
  漏列，而 `spec-00011` §1 已指定本清单为该降级的单一来源）、`spec-00005-FR-8`/`AC-8.1`
...
- **spec-00001-FR-32** (Ubiquitous) 检视面板中的每条需求条目应携带**覆盖状态**，
  ...
  识别。三态之前先定下作用集：一条条目的**计入的 AC**，指该条目下**未标 `作废`**
  的 AC（归属标注的第二个逗号分隔 token，见 `docs/spec/README.md` 的条目文法）；
  标 `作废` 的**墓碑 AC** 仍归属该条目、仍计入 FR-31 的 AC 计数，但不是计入的
  AC。三态按序判定，先命中先出：**未通过**——存在结果非 `pass` 的引用行
  （`fail`、`n/a` 一律入此态；引用其 AC 的行与直接引用条目 id 的行同权）；
  **未覆盖**——该条目没有任何计入的 AC，或存在没有任何引用行的计入的 AC
  （覆盖按 AC 逐条要求；直接引用条目 id 的 `pass` 行不能替代逐 AC 引用，
  decision-00004 §5 裁定三）；**已验证**——其余。墓碑 AC 上的引用行只参与第一
  态：结果非 `pass` 的仍使该条目判「未通过」——作废豁免的是「欠验收」，不豁免
  一条写错或过期的引用，否则一次真实的失败可以靠给 AC 标 `作废` 而消失；结果
  为 `pass` 的**惰性**，不改变任何一态，也不替计入的 AC 交账。覆盖状态以带
  可访问名的图标或文字承载，不只靠颜色传达。
- **spec-00001-FR-33** (Unwanted) 若验收行引用的 AC id 在其所属文档中不存在，
  ...
  正文内的断裂不使文档节点降格为异常节点。**归属标注只认一个可选的第二个
  逗号分隔 token `作废`**（FR-32）：第二个 token 是别的词（拼错或另一个词），
  或标注含三个及以上 token 时，该标注整体不成立，该 AC 同样进「无法归属」区
  ——本条不猜写作者的意图，也不把它当成没标。
...
- **spec-00001-FR-48** (Ubiquitous) 可澄清类型集由代码内建（承载
  ...
  **每个已声明的**类型持有一行焦点行（该类型澄清提问的重心、负向边界与
  ...
  第十二轮扩其承载；配置未声明的
  可澄清类型不存在节点，不要求焦点行）。FR-15 的配置校验扩展（第二十九轮：原「启动校验」）：
...
- **spec-00001-AC-32.11** (spec-00001-FR-32)
  Given 某条目有两条 AC，一条标 `作废`、无任何引用行，另一条被 `pass` 行引用
  When 查看该条目
  Then 其覆盖状态为「已验证」——墓碑 AC 不参与未覆盖判定
- **spec-00001-AC-32.12** (spec-00001-FR-32)
  Given 某条目的未标 `作废` 的 AC 全部有 `pass` 行，而它标 `作废` 的那条 AC
  另有一行结果为 `fail` 的引用行
  When 查看该条目
  Then 其覆盖状态为「未通过」——作废豁免的是「欠验收」，不豁免一条结果非
  `pass` 的引用行
- **spec-00001-AC-32.13** (spec-00001-FR-32)
  Given 某条目的全部 AC 都标了 `作废`，且都没有引用行
  When 查看该条目
  Then 其覆盖状态为「未覆盖」——它没有一条计入的 AC，与零 AC 同判（AC-32.7 的
  口径）
- **spec-00001-AC-32.14** (spec-00001-FR-32)
  Given 某条目有两条 AC，标 `作废` 的那条被 `pass` 行引用，未标的那条没有任何
  引用行
  When 查看该条目
  Then 其覆盖状态为「未覆盖」——墓碑 AC 上的 `pass` 行惰性，不替计入的 AC 交账
...
- **spec-00001-AC-33.4** (spec-00001-FR-33)
  Given 某条 AC 的归属标注第二个 token 不是 `作废`（如 `(spec-00001-FR-1, 作费)`）
  When 选中该文档
  Then 该 AC 出现在解析诊断区（无法归属类），不计入任何条目的 AC 计数、不参与
  该条目的覆盖三态
