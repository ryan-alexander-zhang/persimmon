...
`AC-21.2`、`AC-21.5` 承担。三条仍写成完整的条目声明形态是**故意的**：唯一能让
一行不被解析为验收条目的办法是不以粗体 id 起头，而那会让引它们的 `record` 行
变成断链（`decision-00005` §1 记的正是这一类事故）。**第三十三轮起这不再只靠
本段散文**：三条的归属标注各加了第二个 token `作废`（`docs/spec/README.md` 的
条目文法，`spec-00001-FR-32`），机器据此把它们排除在「计入的 AC」之外，本段与
标记同义（第三十二轮的审计提出墓碑在累积，本段是其处置；审计计作四条，其中
`AC-20.2` 已于第三十二轮复活、重新进入验收集，故此处只列三条）。
...
- **spec-00011-AC-20.3** (spec-00011-FR-20, 作废)
  Given 原断言「本包以不跑安装脚本的缓存安装形态取得后，会话的终端仍起得来」
  （第二十九轮增，`issue-00030`）
  When 该断言的对象是 pty 起得来与否，而不是本条所持的对外标识
  Then 该检查自第三十一轮起退出验收集——**第三十一轮作废，无替代条目**（接手它
  的是一个脚本，不是另一条 AC），其**承载者是
...
- **spec-00011-AC-21.2** (spec-00011-FR-21)
  ...
  When 执行 `persimmon list`
  Then 输出空列表，命令以 0 退出
- **spec-00011-AC-21.3** (spec-00011-FR-21, 作废)
...
- **spec-00011-AC-21.4** (spec-00011-FR-21, 作废)
