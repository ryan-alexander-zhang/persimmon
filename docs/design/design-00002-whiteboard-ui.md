...
```mermaid
flowchart TB
  TB[Top bar<br/>切换器（第二十八轮）· 导航栏开关（第二十四轮）· 标题 · 搜索触发 · 覆盖率总览 · 异常计数 · 诊断计数 · 会话面板（第十六轮）· 桌面通知开关（第十七轮）· 设置入口（第二十六轮，§18）· 主题切换]
  subgraph Work[工作区]
    direction LR
...
**第二十八轮（`spec-00011-FR-7`）**：顶栏最左、导航栏开关之前多一个**切换器**
——当前 workspace 的入口与全部 workspace 的清单。它不占工作区布局：控件形态见 §3 新增的两行，
切换时序、URL、呈现状态分层与页态见
[design-00003](design-00003-multi-workspace.md) §6——切换器的结构由那份持有，
本节只登记它在布局里的位置。注意本节「工作区」一词指导航栏、画布与右槽所在的那块
区域，与 workspace 是两个概念（`CONTEXT.md` 已就此加注）。
...
| 浏览目录（第三十轮） | 无 | 添加对话框中路径输入右侧的 `Button`（`variant="outline"`）：激活即请求 design-00003 §5 的 `POST /api/pick-directory`，服务端弹出**原生**目录选择对话框（浏览器拿不到绝对路径——`webkitdirectory` 只给相对路径、`showDirectoryPicker()` 只给句柄，故必须由服务端出面），回来的路径填进输入框、**不提交**（`spec-00011-FR-22`）。`{path: null}`（取消）静默复位，路径字段不动（`spec-00011-AC-23.1`）；409（已有一个开着）与 503（此处开不了）都以 `toast` 呈现那句说明，输入框照旧可键入并可据以完成添加（`spec-00011-AC-24.1`…`AC-24.3`）。请求在飞时按钮 `disabled`，那**只是**不让同一个页面连点两次——**「至多一个」由服务端持有**（design-00003 §5 的 409，`spec-00011-AC-22.4`）：两个标签页各点一次，按钮管不着。**按钮恒在、不做能力探测**：探测要么多一个端点、要么可能探测成功而真调失败，而失败路径本就要有——恒在加优雅失败是更少的动件；开不了的机器上每次点都给同一句说明（`spec-00011-AC-24.2`），不是第二次起沉默 | `FolderOpen` |
...
**第二十八轮（`spec-00011-FR-8`…`FR-11`）：呈现状态按 workspace 各存一份。**
页面持 `Map<wid, PresentationState>`；切换 workspace 时存下当前那份、装上目标
那份，恢复仍走「按 id 重建」的同一条实现——所指对象已不存在者就近关闭，与
刷新用的是同一段代码，切换因此不是第四个触发来源，而是「换一份状态再重建」。
每会话常驻的 xterm 实例按 `wid:sessionId` 键持有（§12），切换不销毁。
跨页面重载持久的那几项分家：类型组与目录组的展开态按 workspace 分别持久，
导航栏开合与主题、桌面通知开关、面板尺寸是浏览器级偏好、全局一份，当前
workspace 由 URL 承载并另有一个全局键记住「上次所在」——**具体键名与分层由
[design-00003](design-00003-multi-workspace.md) §6 持有**，此处只登记「分层」
这件事。**每个读操作带上发起时的 `wid`，响应回来 `wid` 不等于当前的即丢弃**
——与 `useBoard.ts` 里 `reading` 的串行读队列同源（`issue-00018`：两个无序
读数会丢一次等待）。
...
**第二十八轮（`spec-00011-FR-8`）**：常驻 xterm 实例的键由 `sessionId` 改为
`wid:sessionId`，切换 workspace 只卸载 DOM 容器、不销毁实例——「切回后完整
输出与滚动位置保持」（`spec-00003-AC-5.1`）因此在跨 workspace 切换上同样成立
（`spec-00011-AC-8.3`）。内存上界随之由 `max_sessions` 个实例变为**各已打开
workspace 的上限之和**，仍是单人机器可付的量级。终端 socket 的连接函数多接
一个 `wid`（`connectTerminal(wid, sessionId)`），尺寸帧只从已挂载的实例上报
这一点不变。

## 13. 桌面通知（第十七轮）

承载 `spec-00004` 的界面侧；取舍全部在案于 `decision-00010`。零服务端
改动：事件源就是 §12 会话面板同一份会话载荷（**第二十八轮止**：本轮起事件源
改为 Host 级的 workspace 摘要通路，触发点也与提示条分家——见本节末）。触发点
两处：结束沿 useBoard 既有的结束差分（`announce`，以 `seen` 映射 id → status）；**awaiting
...
- **通知本体**：`new Notification(title, { tag, body })`——**tag 每条
  ...
  即该缺陷的来源）。title/body 只由种类、文档 id、状态与来源 workspace 的
  显示名拼出（显示名系第二十八轮增，`spec-00011-FR-17`），不经手正文。
...
**第二十八轮（`spec-00011-FR-17`）：通知带来源 workspace，两种反馈分家。**

- **单一来源**：通知层的会话集改读 Host 级的 workspace 会话摘要（含当前
  workspace 在内的全部已打开 workspace；通路见
  [design-00003](design-00003-multi-workspace.md) §5、§9），经**一条**有序读
  队列刷新——两条无序通路喂同一张 `seen` 表正是 `issue-00018` 的缺陷类。
  当前 workspace 自己的会话读数仍供会话面板与终端，不参与通知差分。
- **提示条与桌面通知分家**：`announce` 今天一处同时抬两者。改为提示条只随
  当前 workspace 的差分（`spec-00003-FR-7` 的作用域是这块板），桌面通知随
  全部已打开 workspace 的并集差分，元素键 `wid:sessionId`——离场区间判重与
  「后到替换先到」逐键成立，语义不变。
- **基线**：页面首次读到某个 workspace 的会话摘要时（重载后、该 workspace
  首次建实例、从不可用恢复、移除后再添加），那批会话是基线不是新闻——沿
  `announce` 的「首次读数是基线」口径（`useBoard.ts`：`before === undefined`
  即不报）逐 `wid` 成立。转入离场的补发覆盖全部已打开
  workspace 的等待会话，不限当前那个。
- **标题**：由 `<kind> · <sourceId>` 改为 `<name> · <kind> · <sourceId>`，
  正文（状态）不变——外泄面只增加 workspace 显示名一项。
- **点击**：`window.focus()` → `wid` 不是当前则先走
  [design-00003](design-00003-multi-workspace.md) §6 的切换时序（呈现状态的
  存装见 §10；等首屏与会话列表到位）→ 再走本节既有的 `showSession`（含目标不在图上与会话已不在
  的就近处置）；该 workspace 已不可用或已被移除时 `toast` 提示、当前视图不变。
...
- 第二十八轮（多 workspace）：本文档无未决项——切换器的结构、切换时序、
  持久键分层与通知的跨 workspace 组合全部委给
  [design-00003](design-00003-multi-workspace.md)（§6、§9），本文档只登记
  切换器在布局中的位置、§3 的两行控件映射，以及呈现状态按 workspace 分层
  这件事；待域主裁定的三项挂在 design-00003 与 `spec-00011` §8。
